...
export const useKlondikeDrawThreeStore = create<KlondikeStore>()(
  persist(
    (set, get) => ({
      ...
      flipStock: () => {
        ...
        if (state.stock.length > 0) {
          ...
          const { redealtCost, redealsAllowed } = useKlondikeDraw3SettingsStore.getState()
          if (state.redealsUsed >= redealsAllowed) return
...
useKlondikeDrawThreeStore.getState().setHistoryConfig({
  maxDepth: 'unlimited',
