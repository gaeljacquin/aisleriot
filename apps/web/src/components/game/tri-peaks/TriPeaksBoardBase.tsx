...
export default function TriPeaksBoardBase({ useGame, onHowToPlay }: TriPeaksBoardBaseProps) {
  const {
    cells,
    availableCells,
    wasteTop,
      ...
      <div className={cn('relative', isGameOver && 'opacity-50')}>
        ...
        <div className="overflow-x-auto pb-2">
          <PeakGrid
            ...
            onPlayCard={(id) => { lastAction.current = 'play'; onPlayCard(id) }}
          />
        </div>
        ...
        <div className="mt-10 flex items-center justify-center gap-12">
          <Stock
            count={stockCount}
            onClick={() => { lastAction.current = 'draw'; onDraw() }}
            disabled={!canDraw}
          />
          <div ref={wasteRef} className="inline-block">
            <Waste topCard={wasteTop} animate={lastAction.current === 'draw'} />
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-3 py-4">
          <p
            className={cn(
              ...
              effectiveStatus === 'won' ? 'text-green-500' : 'text-red-500',
            )}
          >
