...
if command -v zoxide >/dev/null 2>&1; then
  ...
  for d in /workspaces/aisleriot/apps/*/ /workspaces/aisleriot/packages/*/; do
    if [ -d "$d" ]; then
      zoxide add "$d"
    fi
  done
...
if command -v herdr >/dev/null 2>&1; then
  ...
  setup_tabs_and_panes() {
    ...
    local root_resp
    root_resp=$(herdr tab create --workspace "$ws_id" --label root --cwd /workspaces/aisleriot --no-focus 2>/dev/null || true)
    local p_root
    p_root=$(echo "$root_resp" | jq -r '.result.root_pane.pane_id // empty' 2>/dev/null || true)
    if [ -n "$p_root" ]; then
      herdr pane split "$p_root" --direction right --cwd /workspaces/aisleriot --no-focus >/dev/null 2>&1 || true
    fi

    # Tab 3: apps (web left, api right)
    local apps_resp
    apps_resp=$(herdr tab create --workspace "$ws_id" --label apps --cwd /workspaces/aisleriot/apps/web --no-focus 2>/dev/null || true)
    local p_web
    p_web=$(echo "$apps_resp" | jq -r '.result.root_pane.pane_id // empty' 2>/dev/null || true)
    if [ -n "$p_web" ]; then
      herdr pane split "$p_web" --direction right --cwd /workspaces/aisleriot/apps/api --no-focus >/dev/null 2>&1 || true
    fi

    # Tab 4: packages (2x2 grid: top-left api-client, top-right db, bottom-left shared, bottom-right ui)
    local pkgs_resp
    pkgs_resp=$(herdr tab create --workspace "$ws_id" --label packages --cwd /workspaces/aisleriot/packages/api-client --no-focus 2>/dev/null || true)
    local p_api_client
    p_api_client=$(echo "$pkgs_resp" | jq -r '.result.root_pane.pane_id // empty' 2>/dev/null || true)
    if [ -n "$p_api_client" ]; then
      local p_db
      p_db=$(herdr pane split "$p_api_client" --direction right --cwd /workspaces/aisleriot/packages/db --no-focus 2>/dev/null | jq -r '.result.pane.pane_id // empty' 2>/dev/null || true)
      herdr pane split "$p_api_client" --direction down --cwd /workspaces/aisleriot/packages/shared --no-focus >/dev/null 2>&1 || true
      if [ -n "$p_db" ]; then
        herdr pane split "$p_db" --direction down --cwd /workspaces/aisleriot/packages/ui --no-focus >/dev/null 2>&1 || true
      fi
    fi
  }
  ...
  if [ -n "$WS_ID" ]; then
    herdr workspace rename "$WS_ID" aisleriot >/dev/null 2>&1 || true
    TAB_ID=$(herdr tab list --workspace "$WS_ID" 2>/dev/null | jq -r '.result.tabs[0].tab_id // empty' || true)
    setup_tabs_and_panes "$WS_ID" "$TAB_ID"
  else
    CREATE_RESP=$(herdr workspace create --cwd /workspaces/aisleriot --label aisleriot 2>/dev/null || true)
