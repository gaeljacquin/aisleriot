...
### Step 1: Baseline
Run `nr lint` from the project root. Filter out any results under `packages/ui/` — do not count them, do not fix them, do not auto-fix them.

### Step 2: Auto-Fix Pass
Run `nr lint --fix` first. Let ESLint resolve everything it can automatically (formatting, import order, simple unused vars). Re-run `nr lint` and capture the remaining issues — these require manual fixes.
...
4. After a batch of edits, re-run `nr lint` to confirm the count is decreasing.

### Step 5: Done
When `nr lint` exits cleanly, report:
