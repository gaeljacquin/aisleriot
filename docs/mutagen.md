...
| Variable      | Default              |
| ------------- | -------------------- |
| `LOCAL_PATH`  | `$HOME/aisleriot`      |
| `REMOTE_USER` | `<your-remote-user>` |
| `REMOTE_HOST` | `<your-remote-host>` |
| `REMOTE_PATH` | `<your-remote-path>` |

`PROJECT_NAME` is hardcoded as `aisleriot` in the script and is not configurable via `.env`.
...
| Monitor real-time progress      | `pnpm run sync:monitor`   | `mutagen sync monitor aisleriot`   |
| Pause syncing                   | `pnpm run sync:pause`     | `mutagen sync pause aisleriot`     |
| Resume syncing                  | `pnpm run sync:resume`    | `mutagen sync resume aisleriot`    |
| Stop and remove session         | `pnpm run sync:terminate` | `mutagen sync terminate aisleriot` |
| Reset session (if issues occur) | `pnpm run sync:reset`     | `mutagen sync reset aisleriot`     |
